...
public class LinearRampWidget extends AudioComponentWidget {
    ...
    public LinearRampWidget(AudioComponent ac, AnchorPane parent, String name) {
        ...
        startSlider = new Slider(0, 100, 50);
        stopSlider = new Slider(150, 2000, 500);
        center_.getChildren().addAll(startSlider, stopSlider);
        startSlider.setOnMouseDragged(e->handleSlider());
        stopSlider.setOnMouseDragged(e -> handleSlider());
...
//      Adjusting the title and audiocomponent for changes in the slider
        int start = (int)this.startSlider.getValue();
        int stop = (int)this.stopSlider.getValue();
        this.title.setText("Linear Ramp (Start: " + start + ", Stop: " + stop + ")");
        this.audioComponent_ = new LinearRamp((float)this.startSlider.getValue(), (float)this.stopSlider.getValue());
    }
}
