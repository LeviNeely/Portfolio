...
void World::loop (){
    ...
    while (window.isOpen()){
        ...
        // This variable is used in how frequently an asteroid will spawn in a late part of the code.
        float asteroidSpawnSpeed = 0.5;
        // This variable is used in how many asteroids are allowed on the screen at one time.
        int asteroidMax = 5;
        // This lets the values increase every time 10 seconds has ellapsed in the game.
...
//                    assert((event.key.code == sf::Keyboard::Space)==true);
                    spawnBullet();
                }
            }
        }
...
//            assert(ship.collides(asteroids) == true);
            ship.redraw(origin);
            ship.setPosition(origin);
//            assert(ship.getPosition()==origin);
        }
...
//                    assert(singleBulletCollisionOccurred(bullets[j], asteroids[i])==true);
                    ...
                    // If the asteroid has a radius smaller than 20, it cannot shring, so it removes itself.
        ...
        random_device rd;
        mt19937 gen(rd());
        // Generates a randome number between 0 and 100.
